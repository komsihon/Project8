...
from ikwen_shavida.sales.views import confirm_order
...
urlpatterns = patterns(
    ...
    url(r'^category/(?P<slug>[-\w]+)/$', login_required(MediaList.as_view()), name='media_list'),
    url(r'^contact/$', Contact.as_view(), name='contact'),
    url(r'^bundles/$', login_required(Bundles.as_view()), name='bundles'),
    url(r'^MoMoCheckout/$', MoMoCheckout.as_view(), name='momo_checkout'),
    url(r'^checkout/$', Checkout.as_view(), name='checkout'),
    url(r'^movie/$', login_required(MovieDetail.as_view()), name='movie_detail'),
